# stf-implement core procedure

Shared by the user-facing skill (`SKILL.md`) and the headless subagent (`~/.claude/agents/bd-worker.md`). Neither file duplicates this content — they reference it.
...
When called from `/stf-loop` (not attended) and an implementation fails, the loop must not spin indefinitely. Use:
...
See `bd-human-pause-primitive.md` in this directory for the failure escalation section and the full design.
