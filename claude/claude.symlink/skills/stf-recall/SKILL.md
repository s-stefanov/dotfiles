...
> **Assumes bd is initialized here.** If the repo has no `.beads/`, say so plainly — there's nothing to recall — and offer `/stf-research` instead.
...
Say so plainly and suggest `/stf-research <topic>` to investigate and capture it for next time.
